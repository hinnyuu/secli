...
- 配置文件：白名单覆盖、env 优先于配置、镜像与状态根键、未知键/畸形行拒绝、
  显式 `SECLI_CONFIG` 缺失报错。
- entrypoint：profile/stamp 四种组合、安装失败、回滚、PATH 优先级。
- 命令参数必须用 Bash 数组保存，测试包含空格、引号和以 `-` 开头的 CLI 参数。

当前宿主机启动器和 entrypoint 的自动测试共覆盖 29 个场景；新增 manifest 字段、启动参数
或配置键时必须同步扩展 Bats 覆盖。
...
- 文档统一使用中文：技术标识符、命令、代码、版本号与专有名词保留英文原文；代码及
  代码注释、错误消息文本、workflows、Containerfile 与 LICENSE 不在此列。
